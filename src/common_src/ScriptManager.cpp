...
void ScriptManager::runFunction(std::string script_id, std::string function_name, int argc, v8::Local<v8::Value> argv[], bool reload, std::string context_name) {
	ContextContainer container = context_containers[context_name];
    v8::Context::Scope context_scope(container.context);

	ScriptMetaData smd = container.scripts[script_id];

	v8::Local<v8::Value> obj = container.context->Global()->Get(container.context, v8::String::NewFromUtf8(container.context->GetIsolate(), function_name.c_str(), v8::NewStringType::kInternalized).ToLocalChecked()).ToLocalChecked();

	if (!obj->IsFunction()) {
		// reloadAll();
		std::cout << "object with script_id = " << script_id << " is NOT a function." << std::endl;
		return;
	}

	v8::Local<v8::Function> function = v8::Local<v8::Function>::Cast(obj);

	v8::Local<v8::Value> recv = v8::Local<v8::Value>();

	// v8::Local<v8::Value> args[0];

	function->CallAsFunction(container.context, container.context->Global(), argc, argv);
}
